...
// AudioButton = require('AudioButton')

// we follow a model:
// ->  we won't have two identical named files
...
const AudioLibrary = (function() {
    ...
    function createInstance() {
        const library = {}
        library.filenames = Object.keys(JSON.parse(ipc.sendSync('send-filenames')))
        library.labels = JSON.parse(ipc.sendSync('send-labels')) // filename: label :D
...
const AudioPlayer = (function() {
    ...
    const firstInstance = function() {
        ...
        instance.getAudio = function() { return instance.audio }
        instance.isActive = false
        instance.sections = null
        instance.currPresentation = null
        instance.currImageId = null
        instance.loadAudio = function(button) {
            // calibrating the volume level:
            document.getElementById('volume-bar').style.height = `calc(${instance.audio.volume} * 22px)`
            console.log(button)
            ...
            // we set the required audio-label:
            document.getElementById('audio-label').childNodes[0].innerHTML = button.label
...
document.getElementById('play-button').addEventListener('click', function(event) {
    play = event.target
    play.style.transition = 'border 0.15s, left 0.2s, top 0.2s, width 0.2s, height 0.2s, background-color 0.2s'
    play.style.border = 'none';
    play.style.backgroundColor = 'black'
    play.style.top = '4px'
    play.style.left = '5px'
    play.style.width = '21px'
    play.style.height = '21px'
    play.style.borderRadius = '30px'
    window.setTimeout(function() { play.style = 'transition: border 0.1s, left 0.2s, top 0.2s, width 0.2s, height 0.2s, background-color 0.2s' }, 300)
    console.log(play)
...
var audioTimer = document.createElement('div')
audioTimer.id = 'audio-timer'
audioTimer.class = 'textalignmiddle'
window.setInterval(function() {
        // TODO can be enhanced :D
        if (Math.ceil(audioPlayer.getAudio().duration)) {
            let seconds = Math.ceil(audioPlayer.getAudio().currentTime)
            let curr = `${Math.floor((seconds - seconds % 60) / 60 / 10)}${(seconds - seconds % 60) / 60 % 10}:${Math.floor(seconds % 60 / 10)}${seconds % 60 % 10}`
            seconds = Math.ceil(audioPlayer.getAudio().duration)
            let total = `${Math.floor((seconds - seconds % 60) / 60 / 10)}${(seconds - seconds % 60) / 60 % 10}:${Math.floor(seconds % 60 / 10)}${seconds % 60 % 10}`
            audioTimer.innerHTML = '<span>' + curr + ' /&nbsp;' + total + '</span>'
        } else {
            audioTimer.innerHTML = ''
        }
    }, 100)
var audioText = document.createElement('div')
audioText.id = 'audio-text'
audioText.class = 'textalignmiddle'
audioText.innerHTML = '<span>Once unlocked, text will show here</span>'
textDiv.appendChild(audioTimer)
textDiv.appendChild(audioText)
